...
public class RoundImageView extends ImageView {
    private int mBorderThickness=0;//圆形厚度
    private Context mContext;
    private int defaultColor = 0xFFFFFFFF;//默认圆形框颜色
    ...
    public void setCustomAttributes(AttributeSet attr)
    {
        TypedArray a=mContext.obtainStyledAttributes(attr,R.styleable.roundedimageview);
    ...
    public void onDraw(Canvas canvas)
    {
        ...
        if (mBorderInsideColor != defaultColor && mBorderOutsideColor != defaultColor) {// 定义画两个边框，分别为外圆边框和内圆边框
            radius = (defaultWidth < defaultHeight ? defaultWidth: defaultHeight) / 2 - 2 * mBorderThickness;//设置圆的半径，需减掉设置圆的宽
            // 画内圆
